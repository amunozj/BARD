...
PRO amj_pnr_merge, CRD_in, mdi_i, inar, innar, Rs, ARs, pl_sw
...
;EDITS
PRO kpvt_ar_id, start_date=start_date, continue = continue, restoref = restoref, pnr_file = pnr_file, labl = labl, buff = buff, nobuff = nobuff, kerth1=kerth1, kerth2=kerth2, arth=arth, eros=eros, dila=dila, dislim=dislim, ovrlim=ovrlim, ardislim1=ardislim1, exp_f=exp_f, exp_d=exp_d, exp_s=exp_s, MxFlxim=MxFlxim
...
;seg_const={seg_const, ker_th:200.0, ker_th2:60.0, ar_th:60.0, qr_th:60.0, dila_size:4, eros_size:6.0, npssu: 4, mxB: 0.0, k_sig:15.0, ar_grow_sig:20., valid_range:[-20000.,20000.], deg_lim:70.0}

; EDITS:
;seg_const={seg_const, ker_th:200.0, ker_th2:100.0, ar_th:50.0, ar_th2:50.0, qr_th:60.0, dila_size:2, eros_size:6.0, npssu: 2, dis_lim1:6.0, mxB: 0.0, k_sig:15.0, ar_grow_sig:20., valid_range:[-20000.,20000.], deg_lim:70.0}
;ORIGINAL
;seg_const={seg_const, ker_th:500.0, ker_th2:100.0, ar_th:250.0, ar_th2:50.0, qr_th:60.0, dila_size:2, eros_size:6.0, npssu: 3, dis_lim1:6.0, mxB: 0.0, k_sig:15.0, ar_grow_sig:20., valid_range:[-20000.,20000.], deg_lim:70.0}
;BEST MDI
...
REPEAT BEGIN
  ...
  if (stat eq 1) then begin
      ...
      if ( (detpn_sw eq 1) ) then begin
        ...
        amj_pnr_dt, CRD, mdi_ir, PRs, NRs, seg_const=seg_const;,/not_merge, /detdisp
        detpn_sw = 0   
      endif
...
;      if ( (detar_sw eq 1) and ( n_prs gt 0 ) and ( n_nrs gt 0 ) and ( n_ars eq 0 ) ) then begin
      if ( (detar_sw eq 1) and ( n_prs gt 0 ) and ( n_nrs gt 0 ) ) then begin
        amj_kpvt_coord, mgr.img, hdrr, CRD, seg_const=seg_const                       
        amj_ar_dt_track_dr, CRD, mdi_ir, lbl, PRs, NRs, ARs, ar_cnst=ar_cnst, seg_const=seg_const;,/display;, /bck_trck;, /display
        detar_sw = 0
...
;      pls = 800;
      ...
      if (ds_swl eq 1) then begin
        amj_mgplot, mgl.img, mdi_il, mdi_rf=mdi_ir, hdr_i=hdrl, hdr_f=hdrr, seg_const = seg_const, /tag, ARs = ARs, d_xsize = pls, d_ysize = pls, max = max_sat, pos = [2.0*pad, pad+plxy, pls+2.0*pad, pls+pad+plxy], title = datel, xrange = [min(mgl.x), max(mgl.x)] , yrange = [min(mgl.y), max(mgl.y)]    
      endif
      ;Mixed overlay
      if (ds_swl eq 2) then begin
        amj_mgplot, mgl.img, mdi_il, mdi_rf=mdi_ir, hdr_i=hdrl, hdr_f=hdrr, seg_const = seg_const, /tag, PRs = PRs, NRs = NRs, ARs = ARs, d_xsize = pls, d_ysize = pls, max = max_sat, pos = [2.0*pad, pad+plxy, pls+2.0*pad, pls+pad+plxy], title = datel, xrange = [min(mgl.x), max(mgl.x)] , yrange = [min(mgl.y), max(mgl.y)]    
      endif
      ;PNR overlay
      if (ds_swl eq 3) then begin
        amj_mgplot, mgl.img, mdi_il, mdi_rf=mdi_ir, hdr_i=hdrl, hdr_f=hdrr, seg_const = seg_const, /tag, PRs = PRs, NRs = NRs, d_xsize = pls, d_ysize = pls, max = max_sat, pos = [2.0*pad, pad+plxy, pls+2.0*pad, pls+pad+plxy], title = datel, xrange = [min(mgl.x), max(mgl.x)] , yrange = [min(mgl.y), max(mgl.y)]        
      endif
      ;No overlay
      if (ds_swl eq 4) then begin
        amj_mgplot, mgl.img, mdi_il, mdi_rf=mdi_ir, hdr_i=hdrl, hdr_f=hdrr, seg_const = seg_const, /tag, d_xsize = pls, d_ysize = pls, max = max_sat, pos = [2.0*pad, pad+plxy, pls+2.0*pad, pls+pad+plxy], title = datel, xrange = [min(mgl.x), max(mgl.x)] , yrange = [min(mgl.y), max(mgl.y)]        
      endif
       
      ;Detection Window
      ;Mixed Overlay
      if (ds_swr eq 1) then begin
        amj_mgplot, mgr.img, mdi_ir, mdi_rf=mdi_il, hdr_i=hdrr, hdr_f=hdrl, seg_const = seg_const, /tag, PRs = PRs, NRs = NRs, ARs = ARs, d_xsize = pls, d_ysize = pls, max = max_sat, pos = [pls+4.0*pad, pad+plxy, 2.0*pls+4.0*pad, pls+pad+plxy], title = dater, xrange = [min(mgr.x), max(mgr.x)] , yrange = [min(mgr.y), max(mgr.y)]    
      endif
    ;  ;AR overlay
    ;  if (ds_swr eq 2) then begin
    ;    amj_mgplot, mgr.img, mdi_ir, ARs = ARs, d_xsize = pls, d_ysize = pls, max = max_sat, pos = [pls+4.0*pad, pad+plxy, 2.0*pls+4.0*pad, pls+pad+plxy], title = dater, xrange = [min(mgr.x), max(mgr.x)] , yrange = [min(mgr.y), max(mgr.y)]    
    ;  endif
      ;No overlay
      if (ds_swr eq 2) then begin
        amj_mgplot, mgr.img, mdi_ir, mdi_rf=mdi_il, hdr_i=hdrr, hdr_f=hdrl, seg_const = seg_const, /tag, d_xsize = pls, d_ysize = pls, max = max_sat, pos = [pls+4.0*pad, pad+plxy, 2.0*pls+4.0*pad, pls+pad+plxy], title = dater, xrange = [min(mgr.x), max(mgr.x)] , yrange = [min(mgr.y), max(mgr.y)]    
      endif
              ...
              if (r1_in eq -2) then begin
                ...
                if (r2_in eq -2) then begin
                  ...
                  if (r1_sw*r2_sw lt 0) then begin
                    ...
                    amj_kpvt_coord, mgr.img, hdrr, CRD, seg_const=seg_const                                           
                    amj_ar_manual, CRD, mdi_ir, inp, inn, lbl, PRs, NRs, ARs, ar_cnst=ar_cnst, seg_const=seg_const
                    redraw = 1    
                    und_sw = 1                    
                  endif else begin
              ...
              if (r1_in eq -2) then begin
                ...
                if (r2_in eq -2) then begin
                  ...
                  if (r1_sw*r2_sw lt 0) then begin
                    ...
                    amj_kpvt_coord, mgr.img, hdrr, CRD, seg_const=seg_const                                           
                    amj_ar_manual, CRD, mdi_ir, inp, inn, lbl, PRs, NRs, ARs, ar_cnst=ar_cnst, seg_const=seg_const, /track
                    redraw = 1    
                    und_sw = 1                    
                  endif else begin
                ...
                if (r1_sw*r2_sw gt 0) then begin
                  ...
                  if (r1_sw gt 0) then begin
                    ...
                    if( (PRs[r1_in].lnk_sw eq 1) and (PRs[r2_in].lnk_sw eq 1) ) then begin
                      ...
                      ;Adding region to list and updating PNR indices
                      amj_kpvt_coord, mgr.img, hdrr, CRD, seg_const=seg_const                                           
                      amj_pnr_merge, CRD, mdi_ir, inar, innar, PRs, ARs, pl_sw
                  ...
                  ;Negative regions
                    ...
                    if( (NRs[r1_in].lnk_sw eq 1) and (NRs[r2_in].lnk_sw eq 1) ) then begin
                      ...
                      ;Adding region to list and updating PNR indices
                      amj_kpvt_coord, mgr.img, hdrr, CRD, seg_const=seg_const                                           
                      amj_pnr_merge, CRD, mdi_ir, inar, innar, NRs, ARs, pl_sw
     ...
     16: BEGIN
  	 ...
  	 17: BEGIN
  	        print, 'Jump to'
      			...
      			mg_tmp = amj_kpvt_archive( date, hdrr )
